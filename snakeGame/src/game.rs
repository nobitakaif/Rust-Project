...
use crate::Key::{Up, Down, Right, Left};
use piston_window::*;
...
impl Game {
    ...
    pub fn draw(&self, con:&Context, g: &mut piston_window::wgpu_graphics::WgpuGraphics){
        ...
        if self.game_over{
            draw_rectangle(GAME_COLOR, 0, 0, self.width, self.height, con, g);
        }
    }
    ...
    fn add_food(&mut self){
        ...
        let mut new_y = rng.random_range(1..self.width -1);
        while  self.snake.overlap_tail(new_x, new_y) {
            new_x = rng.random_range(1..self.width -1);
            new_y = rng.random_range(1..self.width -1 );
        }
        self.food_x = new_x;
        self.food_y = new_y;
